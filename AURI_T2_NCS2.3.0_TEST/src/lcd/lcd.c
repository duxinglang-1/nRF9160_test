...
//(x1,y1),(x2,y2):矩形的对角坐标
#if defined(LCD_VGM068A4W01_SH1106G)||defined(LCD_VGM096064A6W01_SP5090)
void LCD_DrawRectExtra(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t color)
{
	uint32_t i;
	uint8_t c=color;
...
void LCD_DrawRectColor(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
#ifdef LCD_TYPE_SPI
	BlockWrite(x,y,w,1);
...
{
#if defined(LCD_VGM068A4W01_SH1106G)||defined(LCD_VGM096064A6W01_SP5090) 
	LCD_DrawRectExtra(x, y, w, h, (uint8_t)(POINT_COLOR&0x00ff));
#else
...
void LCD_ShowEn_from_flash(uint16_t x,uint16_t y,uint8_t num)
{
	...
	uint8_t csize=cbyte*(system_font/2);					//得到字体一个字符对应点阵集所占的字节数	
...
{  							  
	uin8_t cbyte=system_font/8+((system_font%8)?1:0);		//列行扫描，每个字符每一列占用的字节数(英文宽度是字宽的一半)
	uint8_t csize=cbyte*system_font;						//得到字体一个字符对应点阵集所占的字节数	
