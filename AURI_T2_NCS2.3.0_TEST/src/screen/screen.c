...
void DisplayPopUp(uint8_t *message)
{
	uint32_t len;
...
void ShowStringsInRect(uint16_t rect_x, uint16_t rect_y, uint16_t rect_w, uint16_t rect_h, SYSTEM_FONT_SIZE font_size, uint8_t *strbuf)
{
	uint16_t x,y,w,h;
...
void NotifyShowStrings(uint16_t rect_x, uint16_t rect_y, uint16_t rect_w, uint16_t rect_h, SYSTEM_FONT_SIZE font_size, uint8_t *strbuf)
{
	LCD_DrawRectangle(rect_x, rect_y, rect_w, rect_h);
	LCD_Fill(rect_x+1, rect_y+1, rect_w-2, rect_h-2, BLACK);
...
{
	uint16_t rect_x,rect_y,rect_w=180,rect_h=120;
	uint16_t x,y,w,h;
	uint16_t offset_w=4,offset_h=4;
