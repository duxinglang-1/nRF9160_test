...
void MainMenuTimerOutCallBack(struct k_timer *timer_id)
{
	...
	{
		//MenuStartGPS(); //commented beacuse GPS disabled
	}
...
void IdleShowSystemTime(void)
{
	...
	uint16_t x,y,w,h;
	uint8_t str_time_h[4] = {0};
	uint8_t str_time_m[4] = {0};
	uint8_t str_time_dot[4] = {0};
	uint8_t str_ampm[5] = {0};
...
void EnterPoweroffScreen(void)
{
	...
	//if(gps_is_working()) //commented because GPS disabled
	//	MenuStopGPS();
			...
			uint16_t menu_sle_str[LANGUAGE_MAX][3][20] = {
														{
															...
															{0x91CD,0x7F6E,0x0000},//����
															{0x67E5,0x770B,0x0000},//�鿴
														},
														{
			...
			case RESET_STATUS_IDLE:
				{
					uint16_t str_ready[LANGUAGE_MAX][36] = {
															{0x6062,0x590D,0x5230,0x51FA,0x5382,0x8BBE,0x7F6E,0x0000},//�ָ�����������
															...
															{0x0041,0x0075,0x0066,0x0020,0x0057,0x0065,0x0072,0x006B,0x0073,0x0065,0x0069,0x006E,0x0073,0x0074,0x0065,0x006C,0x006C,0x0075,0x006E,0x0067,0x0065,0x006E,0x0020,0x007A,0x0075,0x0072,0x00FC,0x0063,0x006B,0x0073,0x0065,0x0074,0x007A,0x0065,0x006E,0x0000},//Auf Werkseinstellungen zur��cksetzen
														};
			...
			case RESET_STATUS_RUNNING:
				{
					uint16_t str_running[LANGUAGE_MAX][22] = {
															{0x91CD,0x7F6E,0x8FDB,0x884C,0x4E2D,0x0000},//���ý�����
															...
															{0x005A,0x0075,0x0072,0x00FC,0x0063,0x006B,0x0073,0x0065,0x0074,0x007A,0x0065,0x006E,0x0020,0x0069,0x006D,0x0020,0x0047,0x0061,0x006E,0x0067,0x0065,0x0000},//Zur��cksetzen im Gange
														  };
					uint32_t img_addr[8] = {
			...
			case RESET_STATUS_SUCCESS:
				{
					uint16_t str_success[LANGUAGE_MAX][33] = {
															{0x51FA,0x5382,0x8BBE,0x7F6E,0x6062,0x590D,0x6210,0x529F,0x0000},//�������ûָ��ɹ�
			...
			case RESET_STATUS_FAIL:
				{
					uint16_t str_fail[LANGUAGE_MAX][51] = {
															{0x51FA,0x5382,0x8BBE,0x7F6E,0x6062,0x590D,0x5931,0x8D25,0x0000},//�������ûָ�ʧ��
	...
	case SETTINGS_MENU_OTA:
		{
			...
			uint16_t str_notify[LANGUAGE_MAX][27] = {
													{0x5DF2,0x662F,0x6700,0x65B0,0x7248,0x672C,0x0000},//�������°汾
...
void TempUpdateStatus(void)
{
	...
	uint16_t x,y,w,h;
	uint8_t tmpbuf[64] = {0};
	uint32_t img_addr[3] = {IMG_RUNNING_ANI_1_ADDR, IMG_RUNNING_ANI_2_ADDR, IMG_RUNNING_ANI_3_ADDR};
...
void FOTAShowStatus(void)
{
	...
	uint16_t str_notify[LANGUAGE_MAX][40] = {
											{0x7EE7,0x7EED,0x5347,0x7EA7,0x56FA,0x4EF6,0xFF1F,0x0000},//���������̼���
	...
	case FOTA_STATUS_LINKING:
		{
			uint16_t str_notify[LANGUAGE_MAX][26] = {
													{0x8FDE,0x63A5,0x670D,0x52A1,0x5668,0x002E,0x002E,0x002E,0x0000},//���ӷ�����...
													...
													{0x0056,0x0065,0x0072,0x006B,0x006E,0x00FC,0x0070,0x0066,0x0075,0x006E,0x0067,0x0020,0x006D,0x0069,0x0074,0x0020,0x0053,0x0065,0x0072,0x0076,0x0065,0x0072,0x002E,0x002E,0x002E,0x0000},//Verkn��pfung mit Server...
												};
	...
	case FOTA_STATUS_DOWNLOADING:
		if(!flag)
		{
			uint16_t str_notify[LANGUAGE_MAX][32] = {
													{0x6570,0x636E,0x4E0B,0x8F7D,0x4E2D,0x002E,0x002E,0x002E,0x0000},//���������С�
	...
	case FOTA_STATUS_FINISHED:
		{
			uint16_t str_notify[LANGUAGE_MAX][22] = {
													{0x5347,0x7EA7,0x6210,0x529F,0x0000},//�����ɹ�
	...
	case FOTA_STATUS_ERROR:
		{
			uint16_t str_notify[LANGUAGE_MAX][23] = {
													{0x5347,0x7EA7,0x5931,0x8D25,0x0000},//����ʧ��
...
void EnterSleepScreen(void)
{
	...
	if(PPGIsWorking())
		MenuStopPPG();
#endif
	//MenuStopGPS(); //commneted because GPS disabled
...
//enable this function when using GPS
...
void TestGPSScreenProcess(void)
{
	...
	case SCREEN_ACTION_UPDATE:
		//TestGPSUpdateInfor(); \\commented because GPS Disabled
		break;
	}
