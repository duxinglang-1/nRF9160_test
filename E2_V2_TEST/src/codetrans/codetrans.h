...
typedef struct
{
    ...
    const u16_t *pConversnTable;
    u16_t TotalMapTbIndex;
    u8_t Input_word_bits;
    u8_t Output_word_bits;
    u16_t UndefChar;   /* currently taken undefchar as quesmark */
} mmi_chset_info_struct;



extern void mmi_chset_init(void);
extern s32_t mmi_chset_convert(ENUM_MMI_CHSET src_type, ENUM_MMI_CHSET dest_type, char *src_buff, char *dest_buff, s32_t dest_size);
