...
static int inject_agps_type(void *agps, size_t agps_size, uint16_t type, void *user_data)
{
	...
	{
	#ifdef AGPS_DEBUG
		LOGD("Failed to write A-GPS data, type: %d (errno: %d)", type, errno);
	#endif
		return -1;
	}

#ifdef AGPS_DEBUG
	LOGD("Injected A-GPS data, type: %d, size: %d", type, agps_size);
#endif
...
static int supl_logger(int level, const char *fmt, ...)
{
	...
	{
	#ifdef AGPS_DEBUG
		LOGD("%s: too long message," "it will be cut short");
	#endif
	}
...
static int open_supl_socket(void)
{
	...
	{
	#ifdef AGPS_DEBUG
		LOGD("Failed to resolve hostname %s, error: %d", SUPL_SERVER, err);
	#endif
	...
	for(struct addrinfo *addr = info; addr != NULL; addr = addr->ai_next)
	{
		...
		{
		#ifdef AGPS_DEBUG
			LOGD("Failed to create socket, errno %d", errno);
		#endif
			goto cleanup;
		}
		...
		{
		#ifdef AGPS_DEBUG
			LOGD("Failed to set socket timeout, errno %d", errno);
		#endif
			goto cleanup;
		}
	...
	#ifdef AGPS_DEBUG
		LOGD("Connecting to %s port %d", ip, SUPL_SERVER_PORT);
	#endif
		...
		#ifdef AGPS_DEBUG
			LOGD("Connecting to server failed, errno %d", errno);
		#endif
		}
		else
		{
...
cleanup:
	...
	#ifdef AGPS_DEBUG
		LOGD("Could not connect to SUPL server");
	#endif
		if(supl_fd > -1)
		{
			close(supl_fd);
...
static void close_supl_socket(void)
{
	...
	{
	#ifdef AGPS_DEBUG
		LOGD("Failed to close SUPL socket");
	#endif
	}
}
...
int assistance_init(struct k_work_q *assistance_work_q)
{
	...
	{
	#ifdef AGPS_DEBUG
		LOGD("Failed to initialize SUPL library");
	#endif
		return -1;
	}
