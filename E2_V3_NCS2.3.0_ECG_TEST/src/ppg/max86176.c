...
static void PPG_gpio_Init(void)
{
	...
	// 设置为高，模式为测量ecg
	gpio_pin_configure(gpio_ppg, ECG_PWR_H_PIN, GPIO_OUTPUT);
	gpio_pin_set(gpio_ppg, ECG_PWR_H_PIN, 1);

	gpio_pin_configure(gpio_ppg, ECG_PWR_L_PIN, GPIO_OUTPUT);
	gpio_pin_set(gpio_ppg, ECG_PWR_L_PIN, 0);

	gpio_pin_configure(gpio_ppg, ECG_SW_PIN, GPIO_OUTPUT);
	gpio_pin_set(gpio_ppg, ECG_SW_PIN, 1);
