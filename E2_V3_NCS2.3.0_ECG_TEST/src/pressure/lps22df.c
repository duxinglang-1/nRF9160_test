...
void LPS22DF_SetInterruptCfg(void)
{
	...
	LPS22DF_WriteReg(REG_INTERRUPT_CFG, 0x00);
}
...
void LPS22DF_SetThrePrs(uint16_t data)
{
	//THS_P_L:THS[7:0]
	//THS_P_H:THS[14:8]
	uint8_t ths_p[2] = {0};

	ths_p[0] = (uint8_t)(data&0x00ff);
	ths_p[1] = (data>>8);
...
void LPS22DF_SetCtrl1(uint8_t data)
{
	//Control register 1
	//b7	b6b5b4b3	b2b1b0
	//0		ODR[3:0]	AVG[2:0]
	...
	LPS22DF_WriteReg(REG_CTRL_REG1, data);
}
...
void LPS22DF_SetCtrl2(uint8_t data)
{
	//Control register 2
	//b7	b6	b5			b4		b3	b2		b1	b0
...
void LPS22DF_SetCtrl3(uint8_t data)
{
	//Control register 3
	//b7b6b5b4	b3		b2	b1		b0
	//	 0		INT_H_L 0 	PP_OD 	IF_ADD_INC
...
void LPS22DF_SetCtrl4(uint8_t data)
{
	//Control register 4
	//b7	b6			b5		b4		b3	b2			b1			b0
...
void LPS22DF_SetFIFOCtrl(uint8_t data)
{
	//FIFO control register
	//b7b6b5b4	b3			b2			b1b0
	//	 0 		STOP_ON_WTM TRIG_MODES 	F_MODE[1:0]
...
void LPS22DF_SetFIFOWTM(uint8_t data)
{
...
void LPS22DF_SetREFP(uint16_t data)
{
	//Reference pressure
	//REF_P_L:REFL[7:0]
	//REF_P_H:REFH[15:8]
...
void LPS22DF_SetI3CCtrl(uint8_t data)
{
	//I3C interface Control register
	//b7	b6	b5		b4b3b2	b1b0
	//1 	0 	ASF_ON 	  0 	I3C_Bus_Avb_Sel[1:0]
	...
	LPS22DF_WriteReg(REG_I3C_IF_CTRL, (data&0x7f));
}
...
void LPS22DF_SetPRDS(uint16_t data)
{
	//Pressure offset
	//RPDS_L:RPDS[7:0]
	//RPDS_H:RPDS[15:8]
	...
	offset_p[0] = (uint8_t)(data&0x00ff);
	offset_p[1] = (data>>8);
...
void LPS22DF_Start(LPS22DF_MEAS_CTRL meas_type)
{
	...
	case MEAS_ONE_SHOT:
		//set ODR
		LPS22DF_SetCtrl1(0x00);
		//set interrupt active mode
		LPS22DF_SetCtrl3(0x09);
		//set interrupt active event
		LPS22DF_SetCtrl4(0x77);
		//set fifo ctrl
		LPS22DF_SetFIFOCtrl(0x00);
		//start a single measurement of pressure and temperature
		LPS22DF_SetCtrl2(0x01);
		break;
