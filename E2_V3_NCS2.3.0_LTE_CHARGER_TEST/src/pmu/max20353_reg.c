...
int WriteWord(uint8_t reg, uint8_t MSB, uint8_t LSB);
int WriteMulti(uint8_t *data, uint8_t len);
#endif

static uint8_t HardwareID;
static uint8_t FirmwareID;

static uint8_t PMICIntMasks[3];
...
int MAX20353_ChargerCfg(void)
{
	...
	appdatainoutbuffer_[0] = 0x04; // Maintain charge b00:0min, FastCharge b01:150min, for 1C charging, PreCharge b00: 30min for dead battery 
...
#if 0
	appcmdoutvalue_ = 0x16; 
	appdatainoutbuffer_[0] = 0xC6; // ColdLim[7:0]: 0xFF*(1.39/1.8) -5度
	appdatainoutbuffer_[1] = 0xBB; // CoolLim[7:0]: 0xFF*(1.313/1.8) 0度
	appdatainoutbuffer_[2] = 0x58; // WarmLim[7:0]: 0xFF*(0.64/1.8)  45度
	appdatainoutbuffer_[3] = 0x49; // HotLim[7:0]:	0xFF*(0.514/1.8) 50度
	appdatainoutbuffer_[4] = 0x1E; // Password[15:8]	0x1E7A
	appdatainoutbuffer_[5] = 0x7A; // Password[7:0]
	ret |= MAX20353_AppWrite(6);

	appcmdoutvalue_ = 0x18; 
	appdatainoutbuffer_[0] = 0x00; // ColdChgEn[7] b0:Charging disabled in cold temperature zone, ColdBatReg[4:3] b00:BatReg-150mV, ColdFChg[2:0] b000:0.2 x IFChg
	appdatainoutbuffer_[1] = 0x8A; // CoolChgEn[7] b1:Charging enabled in cool temperature zone, CoolBatReg[4:3] b01:BatReg-100mV, CoolFChg[2:0] b010:0.4 x IFChg
	appdatainoutbuffer_[2] = 0x1F; //															RoomBatReg[4:3] b11:BatReg, RoomFChg[2:0] b111:1.0 x IFChg
	appdatainoutbuffer_[3] = 0x9F; // WarmChgEn[7] b1:Charging enabled in warm temperature zone, WarmBatReg[4:3] b11:BatReg, WarmFChg[2:0] b111:1.0 x IFChg
	appdatainoutbuffer_[4] = 0x1F; // HotChgEn[7] b0:Charging disabled in hot temperature zone, HotBatReg[4:3] b11:BatReg, HotFChg[2:0] b111:1.0 x IFChg
	appdatainoutbuffer_[5] = 0x1E; // Password[15:8]	0x1E7A
	appdatainoutbuffer_[6] = 0x7A; // Password[7:0]
	ret |= MAX20353_AppWrite(7);
#endif
...
//充电禁止
int MAX20353_ChargerDisable(void)
{
	int32_t ret = 0;

	appcmdoutvalue_ = 0x1A; 
	appdatainoutbuffer_[0] = 0x00; // Thermal EN, Charger EN 
	ret |= MAX20353_AppWrite(1); 
...
int MAX20353_ChargerCtrl(void)
{
	...
	appcmdoutvalue_ = 0x1A; 
	appdatainoutbuffer_[0] = 0x03; // Thermal EN, Charger EN 
	ret |= MAX20353_AppWrite(1); 
...
void MAX20353_GetDeviceID(uint8_t *Device_ID)
{
...
void MAX20353_UpdateTemper(void)
{
	...
	static int16_t pre_temper=20;	//初始化默认20度
	
	thm = MAX20353_ReadTHM();
#ifdef PMU_DEBUG
	//LOGD("thm:%x", thm);
#endif
	if(thm == pre_thm || thm == 0)
		return;
...
void MAX20353_StartCheckTemper(void)
{
	...
	k_timer_start(&ntc_check_timer, K_MSEC(2*1000), K_MSEC(2*1000));
}

#endif/*BATTERT_NTC_CHECK*/

uint8_t MAX20353_CalculateSOC(void)
{
	uint16_t tmp;
...
void MAX20353_NewInitVoltage(void)
{
	...
	uint16_t VCell1, VCell2, OCV,Desired_OCV;
...
void prepare_to_load_model(void)
{
	uint8_t unlock_test_OCV_1, unlock_test_OCV_2;
...
int MAX20353_SOCReadReg(uint8_t reg, uint8_t *MSB, uint8_t *LSB)
{
	uint32_t ret;
	uint8_t data = reg;
	uint8_t value[2];
...
int MAX20353_SOCWriteReg(uint8_t reg, uint8_t MSB, uint8_t LSB)
{
	uint8_t cmdData[3] = {reg, MSB, LSB};
	uint32_t rslt = 0;
