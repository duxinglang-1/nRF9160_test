...
bool add_data_into_cache(CacheInfo *data_cache, uint8_t *data, uint32_t len, DATA_TYPE type)
{
	DataNode *pnew;
...
bool get_data_from_cache(CacheInfo *data_cache, uint8_t **buf, uint32_t *len, DATA_TYPE *type)
{
	if(data_cache->cache == NULL || data_cache->count == 0)
	{
		buf = NULL;
