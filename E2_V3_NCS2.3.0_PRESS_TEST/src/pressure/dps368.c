...
static void pre_init_confirm_timerout(struct k_timer *timer_id);
K_TIMER_DEFINE(pre_init_timer, pre_init_confirm_timerout, NULL);
...
bool DPS368_Init(void)
{
	...
	if(chip_id == DPS368_CHIP_ID)
	{
		...
		*/
		...
		k_timer_start(&pre_init_timer, K_MSEC(50), K_NO_WAIT);
		/*
		DPS368_SetMeasTmpCfg();
		DPS368_SetMeasPrsCfg();
		DPS368_GetCoefs();
