...
uint16_t *GetStrDataFromFlashByID(RES_STRINGS_ID str_id)
{
	...
	{
		SpiFlash_Read((uint8_t)data, STR_DATA_ADDR, 4);
		strlib_infor.str_count = (data[1]<<8) + data[0];
		strlib_infor.lang_count = (data[3]<<8) + data[2];
		len = strlib_infor.str_count*strlib_infor.lang_count*sizeof(dataindex);
		strlib_infor.p_index = k_malloc(len);
		if(strlib_infor.p_index)
		{
			SpiFlash_Read((uint8_t)strlib_infor.p_index, STR_DATA_ADDR+4, len);
		}
	}

	ptr_index = strlib_infor.p_index;
	for(i=0;i<(str_id*strlib_infor.lang_count);i++)
		ptr_index++;
	...
	if(ptr_data)
	{
		...
		SpiFlash_Read((uint8_t)ptr_data, STR_DATA_ADDR+4+len+ptr_index->addr, ptr_index->len);
		return ptr_data;
	}
	else
	{
