...
static uint16_t still_str[LANGUAGE_MAX][21] = {
												...
												{0x0627,0x0628,0x0642,0x0649,0x0020,0x0633,0x0627,0x0643,0x0646,0x0627,0x0000},//???? ?????
											#else
...
static uint16_t Incon_1_str[LANGUAGE_MAX][40] = {
													...
													{0x004F,0x0074,0x0079,0x0064,0x006C,0x0069,0x0067,0x0074,0x002C,0x0020,0x0066,0x00F6,0x0072,0x0073,0x00F6,0x006B,0x0020,0x0073,0x0065,0x006E,0x0061,0x0072,0x0065,0x002E,0x0000},//Otydligt, f?rs?k senare.
													...
													{0x0625,0x0639,0x0627,0x062F,0x0629,0x0020,0x0627,0x0644,0x0645,0x062D,0x0627,0x0648,0x0644,0x0629,0x0020,0x0644,0x0627,0x062D,0x0642,0x064B,0x0627,0x0020,0x063A,0x064A,0x0631,0x0020,0x062D,0x0627,0x0633,0x0645,0x0629,0x0000},//????? ???????? ?????? ??? ?????
												#else
...
static uint16_t Incon_2_str[LANGUAGE_MAX][18] = {
													...
													{0x063A,0x064A,0x0631,0x0020,0x062D,0x0627,0x0633,0x0645,0x0000},//??? ????
												#else
...
static uint16_t still_retry_str[LANGUAGE_MAX][42] = {
														...
														{0x0056,0x0061,0x0072,0x0020,0x0073,0x0074,0x0069,0x006C,0x006C,0x0061,0x0020,0x006F,0x0063,0x0068,0x0020,0x0066,0x00F6,0x0072,0x0073,0x00F6,0x006B,0x002E,0x0000},//Var stilla och f?rs?k.
														...
														{0x0627,0x0628,0x0642,0x0020,0x0633,0x0627,0x0643,0x0646,0x064B,0x0627,0x0020,0x0648,0x062D,0x0627,0x0648,0x0644,0x0020,0x0645,0x0631,0x0629,0x0020,0x0623,0x062E,0x0631,0x0649,0x0000},//??? ?????? ????? ??? ????
													#else
...
void IdleShowSystemDate(void)
{
	...
	default:
		...
		if(g_language_r2l)
			LCD_ShowUniString(IDLE_DATE_MON_EN_X+IDLE_DATE_MON_EN_W, IDLE_DATE_MON_EN_Y, tmpbuf);
		else
...
void IdleShowSystemWeek(void)
{
	...
	uint16_t str_week[LANGUAGE_MAX][7][13] = {
												...
												{//Arabic
													...
													{0x0627,0x0644,0x0627,0x062B,0x0646,0x064A,0x0646,0x0000},//???????
													{0x064A,0x0648,0x0645,0x0020,0x0627,0x0644,0x062B,0x0644,0x0627,0x062B,0x0627,0x0621,0x0000},//??? ????????
													{0x0627,0x0644,0x0623,0x0631,0x0628,0x0639,0x0627,0x0621,0x0000},//????????
													{0x064A,0x0648,0x0645,0x0020,0x0627,0x0644,0x062E,0x0645,0x064A,0x0633,0x0000},//??? ??????
	...
	if(g_language_r2l)
		LCD_ShowUniString(x+w-str_w, y, tmpbuf);
	else
	...
	case SETTINGS_MENU_MAIN:
		{
			...
			uint16_t menu_sle_str[LANGUAGE_MAX][3][14] = {
															...
															{
																{0x0645,0x0646,0x0638,0x0631,0x0000},//????
																{0x0000},//null
			...
			for(i=0;i<SETTINGS_MAIN_MENU_MAX_PER_PG;i++)
			{
				...
				if(g_language_r2l)
					LCD_ShowUniString(LCD_WIDTH-(SETTINGS_MENU_BG_X+SETTINGS_MENU_STR_OFFSET_X),
				...
				case 0:
					...
					case 0:
						...
						LCD_MeasureUniString(tmpbuf, &w, &h);
						if(g_language_r2l)
							LCD_ShowUniString(LCD_WIDTH-(SETTINGS_MENU_BG_X+SETTINGS_MENU_BG_W-SETTINGS_MENU_STR_OFFSET_X-w),
												SETTINGS_MENU_BG_Y+i*(SETTINGS_MENU_BG_H+SETTINGS_MENU_BG_OFFSET_Y)+(SETTINGS_MENU_BG_H-h)/2,
												tmpbuf);
						else
			...
			case RESET_STATUS_IDLE:
				{
					uint16_t str_ready[LANGUAGE_MAX][44] = {
																...
																{0x0639,0x062F,0x0627,0x062F,0x0627,0x062A,0x0020,0x0627,0x0644,0x0645,0x0635,0x0646,0x0639,0x0625,0x0639,0x0627,0x062F,0x0629,0x0020,0x0627,0x0644,0x0636,0x0628,0x0637,0x0020,0x0625,0x0644,0x0649,0x0020,0x0625,0x0000},//????? ????? ??? ??????? ??????
															#else
					...
					LCD_MeasureUniString(tmpbuf, &w, &h);
					if(g_language_r2l)
						LCD_ShowUniString(LCD_WIDTH-(SETTINGS_MENU_RESET_STR_X+(SETTINGS_MENU_RESET_STR_W-w)/2), 
			...
			case RESET_STATUS_RUNNING:
				{
					uint16_t str_running[LANGUAGE_MAX][28] = {
																...
																{0x062C,0x0627,0x0631,0x064A,0x0020,0x0625,0x0639,0x0627,0x062F,0x0629,0x0020,0x0627,0x0644,0x0636,0x0628,0x0637,0x0000},//???? ????? ?????
															#else
					...
					LCD_MeasureUniString(tmpbuf, &w, &h);
					if(g_language_r2l)
						LCD_ShowUniString(LCD_WIDTH-(SETTINGS_MENU_RESET_NOTIFY_X+(SETTINGS_MENU_RESET_NOTIFY_W-w)/2), 
			...
			case RESET_STATUS_SUCCESS:
				{
					uint16_t str_success[LANGUAGE_MAX][26] = {
																...
																{0x0646,0x062C,0x0627,0x062D,0x062A,0x0645,0x062A,0x0020,0x0625,0x0639,0x0627,0x062F,0x0629,0x0020,0x0627,0x0644,0x0636,0x0628,0x0637,0x0020,0x0628,0x0000},//??? ????? ????? ?????
															#else
			...
			case RESET_STATUS_FAIL:
				{
					uint16_t str_fail[LANGUAGE_MAX][30] = {
																...
																{0x0641,0x0634,0x0644,0x062A,0x0020,0x0625,0x0639,0x0627,0x062F,0x0629,0x0020,0x0627,0x0644,0x0636,0x0628,0x0637,0x0000},//???? ????? ?????
															#else
	...
	case SETTINGS_MENU_OTA:
		{
			...
			uint16_t str_notify[LANGUAGE_MAX][30] = {
														...
														{0x0625,0x0646,0x0647,0x0627,0x0020,0x0627,0x0644,0x0646,0x0633,0x062E,0x0629,0x0020,0x0627,0x0644,0x0623,0x062D,0x062F,0x062B,0x0000},//???? ?????? ??????
													#else
			...
			LCD_MeasureUniString(tmpbuf, &w, &h);
			if(g_language_r2l)
				LCD_ShowUniString(LCD_WIDTH-(LCD_WIDTH-w)/2, (LCD_HEIGHT-h)/2, tmpbuf);
			else
...
void SettingsShowStatus(void)
{
	...
	uint16_t menu_sle_str[LANGUAGE_MAX][11] = {
												...
												{0xD55C,0xAD6D,0xC778,0x0000},//???
												{0x0420,0x0443,0x0441,0x0441,0x043A,0x0438,0x0439,0x0000},//Русский
												{0x0639,0x0631,0x0628,0x064A,0x0000},//????
											#else
												{0x4E2D,0x6587,0x0000},//中文
												{0x0045,0x006E,0x0067,0x006C,0x0069,0x0073,0x0068,0x0000},//English
											#endif	
...
void TempShowStatus(void)
{
	...
	uint16_t title_str[LANGUAGE_MAX][24] = {
												...
												{0x062F,0x0631,0x062C,0x0629,0x0020,0x062D,0x0631,0x0627,0x0631,0x0629,0x0020,0x0627,0x0644,0x062C,0x0633,0x0645,0x0000},//???? ????? ?????
											#else
												{0x4F53,0x6E29,0x0000},//体温
												{0x0042,0x006F,0x0064,0x0079,0x0020,0x0054,0x0065,0x006D,0x0070,0x0065,0x0072,0x0061,0x0074,0x0075,0x0072,0x0065,0x0000},//Body Temperature
											#endif
...
void SPO2ShowStatus(void)
{
	...
	uint16_t title_str[LANGUAGE_MAX][25] = {
												...
												{0x0627,0x0644,0x0623,0x0643,0x0633,0x062C,0x064A,0x0646,0x0020,0x0641,0x064A,0x0020,0x0627,0x0644,0x062F,0x0645,0x0000},//???????? ?? ????
											#else
...
void HRShowStatus(void)
{
	...
	uint16_t title_str[LANGUAGE_MAX][29] = {
												...
												{0x0645,0x0639,0x062F,0x0644,0x0020,0x0636,0x0631,0x0628,0x0627,0x062A,0x0020,0x0627,0x0644,0x0642,0x0644,0x0628,0x0000},//???? ????? ?????
											#else
...
uint16_t dl_idle_notify_ar[] =
{
	...
	0x0644,0x0628,0x0637,0x0627,0x0631,0x064A,0x0629,0x0020,0x0623,0x0639,0x0644,0x0649,0x0020,0x0645,0x0646,0x0020,0x0038,0x0030,
	0x0025,0x0020,0x0623,0x0648,0x0020,0x0623,0x0646,0x0020,0x0643,0x0627,0x0628,0x0644,0x0020,0x0627,0x0644,0x0634,0x062D,0x0646,
			...
			uint16_t str_linking[LANGUAGE_MAX][33] = 
				{
					...
					{0x0627,0x0644,0x0627,0x0631,0x062A,0x0628,0x0627,0x0637,0x0020,0x0628,0x0627,0x0644,0x062E,0x0627,0x062F,0x0645,0x002E,0x002E,0x002E,0x0000},//???????? ???????...
				#else
	...
	case DL_STATUS_DOWNLOADING:
		if(!flag)
		{
			uint16_t str_downloading[LANGUAGE_MAX][31] = {
													...
													{0x062C,0x0627,0x0631,0x064A,0x0020,0x0627,0x0644,0x062A,0x0631,0x0642,0x064A,0x0629,0x002E,0x002E,0x002E,0x0000},//???? ???????...
											#else
