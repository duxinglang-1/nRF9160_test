...
{
	uint8_t tmpbuf[8] = {0};
	uint8_t MSB,LSB,RCOMP_H,RCOMP_L,VReset,Status;
	float vcell,soc,CRate;
	uint16_t ver,hibrt,valrt,ID,cmd,MODE;

	if(!pmu_check_ok)
		return;

	//VCELL
	MAX20353_SOCReadReg(0x02, &MSB, &LSB);
	vcell = (78.125*(256*MSB+LSB))/1000000.0;
	//SOC
	MAX20353_SOCReadReg(0x04, &MSB, &LSB);
	soc = (256*MSB+LSB)/256.0;
	//RCOMP
	MAX20353_SOCReadReg(0x0C, &RCOMP_H, &RCOMP_L);
	//MODE
	MAX20353_SOCReadReg(0x06, &MSB, &LSB);
	MODE = (256*MSB+LSB);
	//VER
	MAX20353_SOCReadReg(0x08, &MSB, &LSB);
	ver = (256*MSB+LSB);
	//HIBRT
	MAX20353_SOCReadReg(0x0A, &MSB, &LSB);
	hibrt = (256*MSB+LSB);
	//VALRT
	MAX20353_SOCReadReg(0x14, &MSB, &LSB);
	valrt = (256*MSB+LSB);
	//CRate
	MAX20353_SOCReadReg(0x16, &MSB, &LSB);
	CRate = 1.0*(256*MSB+LSB);
	//VRest
	MAX20353_SOCReadReg(0x18, &MSB, &LSB);
	VReset = MSB;
	//ID
	MAX20353_SOCReadReg(0x19, &MSB, &LSB);
	ID = (256*MSB+LSB);
	//Status
	MAX20353_SOCReadReg(0x1A, &MSB, &LSB);
	Status = MSB;
	//CMD
	MAX20353_SOCReadReg(0xFE, &MSB, &LSB);
	cmd = (256*MSB+LSB);

#ifdef PMU_DEBUG
	LOGD("%02d-%02d-%04d %02d:%02d:%02d,%d.%05d,%d.%05d,0x%02X,0x%04X,0x%04X,0x%04X,0x%04X,0x%04X,0x%04X,0x%02X,0x%04X,0x%02X,0x%04X", date_time.day,date_time.month,date_time.year,date_time.hour,date_time.minute,date_time.second,
												((uint32_t)(vcell*100000))/100000,((uint32_t)(vcell*100000))%100000,
												((uint32_t)(soc*100000))/100000,((uint32_t)(soc*100000))%100000,
												RCOMP_H,
												MODE,
												ver,
												hibrt,
												(0xff*RCOMP_H+RCOMP_L),
												valrt,
												((uint16_t)(CRate*10))/10,
												VReset,
												ID,
												Status,
												cmd);
