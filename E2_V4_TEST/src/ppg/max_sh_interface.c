...
#ifdef GPIO_ACT_I2C
	rslt = I2C_write_data(MAX32674_I2C_ADD, bufp, len);
#else
	rslt = i2c_write(handle, bufp, len, MAX32674_I2C_ADD);
#endif
	return rslt;
}
...
#ifdef GPIO_ACT_I2C
	rslt = I2C_read_data(MAX32674_I2C_ADD, bufp, len);
#else
	rslt = i2c_read(handle, bufp, len, MAX32674_I2C_ADD);
#endif
	return rslt;
}
				...
				int sleep_ms )
{
	...
	int ret, retries = SS_DEFAULT_RETRIES;

	SH_mfio_to_low_and_keep(MFIO_LOW_DURATION);
	ret = ppg_dev_ctx.write(i2c_ppg, cmd_bytes, cmd_bytes_len);
	...
	SH_mfio_to_low_and_keep(MFIO_LOW_DURATION);
	ret = ppg_dev_ctx.read(i2c_ppg, rxbuf, rxbuf_sz);
	SH_pull_mfio_to_high();
				 				   ...
				 				   int sleep_ms)
{
	...
	SH_mfio_to_low_and_keep(MFIO_LOW_DURATION);
	int ret = ppg_dev_ctx.write(i2c_ppg, tx_buf, tx_len);
	SH_pull_mfio_to_high();
	...
	while((ret != 0) && (retries--) > 0)
	{
		WAIT_MS(1);
		SH_mfio_to_low_and_keep(MFIO_LOW_DURATION);
		ret = ppg_dev_ctx.write(i2c_ppg, tx_buf, tx_len);
		SH_pull_mfio_to_high();
	}
				 ...
				 int sleep_ms)
{
	...
	uint8_t status_byte;
	SH_mfio_to_low_and_keep(MFIO_LOW_DURATION);
	ret = ppg_dev_ctx.read(i2c_ppg, (uint8_t*)&status_byte, 1);
	SH_pull_mfio_to_high();
