...
{
	u8_t strbuf[128] = {0};
	static bool wearing_flag = false;
	...
	if(int2_event)	//tilt
	{
		...
		if(fall_result)
		{
			...
			sprintf(strbuf, "fall!!!");
			LCD_ShowString(40,180,strbuf);
		}
		else
		{
			sprintf(strbuf,"Not fall");
			LCD_ShowString(40,180,strbuf);
		}
	}
	...
	if(is_wearing())
	{
		if(!wearing_flag)
		{
			...
			sprintf(strbuf,"take on!");
			LCD_ShowString(120,180,strbuf);
		}
	}
	else
	{
