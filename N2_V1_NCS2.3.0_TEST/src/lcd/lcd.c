...
#elif defined(LCD_JX154QV24BT_ST7789V)
...
void LCD_FillColor(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t color)
{          
	uint32_t i;
...
uint8_t LCD_Show_Uni_Char_from_flash(uint16_t x, uint16_t y, uint16_t num, uint8_t mode)
{
	...
	for(t=0;t<csize;t++)
	{
		...
		for(t1=0;t1<8;t1++)
		{
			...
			{
			#ifdef LCD_PIXEL_DEPTH_24
				databuf[dot_count*i+0] = POINT_COLOR>>16;
				databuf[dot_count*i+1] = POINT_COLOR>>8;
				databuf[dot_count*i+2] = POINT_COLOR;
			#elif defined(LCD_PIXEL_DEPTH_16)
				databuf[dot_count*i+0] = POINT_COLOR>>8;
				databuf[dot_count*i+1] = POINT_COLOR;
			#endif	
			}
			else if(mode==0)
			{
			#ifdef LCD_PIXEL_DEPTH_24
				databuf[dot_count*i+0] = BACK_COLOR>>16;
				databuf[dot_count*i+1] = BACK_COLOR>>8;
				databuf[dot_count*i+2] = BACK_COLOR;
			#elif defined(LCD_PIXEL_DEPTH_16)
				databuf[dot_count*i+0] = BACK_COLOR>>8;
				databuf[dot_count*i+1] = BACK_COLOR;
			#endif	
			}
