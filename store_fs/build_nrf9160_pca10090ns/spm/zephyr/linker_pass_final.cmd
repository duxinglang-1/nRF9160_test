...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x10200, LENGTH = 0xc000
...
.gnu.sgstubs :
{
 . = ALIGN(0x8000) - (1 << LOG2CEIL(__sg_size));
 __sg_start = .;
} > FLASH
__sg_end = .;
...
__nsc_size = . - __sg_start;
 ASSERT((__sg_size == 0)
  || (((1 << LOG2CEIL((0x8000 - (__sg_start % 0x8000)))) == (0x8000 - (__sg_start % 0x8000)))
   && ((0x8000 - (__sg_start % 0x8000)) >= 32)
   && ((0x8000 - (__sg_start % 0x8000)) <= 4096)),
  "The Non-Secure Callable region size must be a power of 2 between 32 and 4096 bytes.")
 .stab 0 : { *(.stab) }
 .stabstr 0 : { *(.stabstr) }
